...
class AppleMusicService {
  ...
  Future<Playlists> getStorefrontPlaylist() async {
    final Map<String, String> queryParameters = {
      "filter[storefront-chart]": AppleMusicToken.countryCode
    };
    final uri = _generateUrl(
        '/v1/catalog/${AppleMusicToken.countryCode}/playlists',
        queryParameters);
  ...
  Future<ChartResponse> getEditorial({int? limit}) async {
    ...
    final uri = _generateUrl(
        '/v1/catalog/${AppleMusicToken.countryCode}/charts', queryParameters);
    return await appleMusicDio.dio.getUri(uri).then((value) {
  ...
  Future<List<Playlists>> getMostPlayedPlaylist({int limit = 10}) async {
    final String url =
        "/v1/catalog/${AppleMusicToken.countryCode}/charts?chart=most-played&limit=$limit&types=playlists";
  ...
  Future<List<Albums>> getMostPlayedAlbums({int limit = 10}) async {
    final String url =
        "/v1/catalog/${AppleMusicToken.countryCode}/charts?chart=most-played&limit=$limit&types=albums";
    return await appleMusicDio.dio
        .getUri(
      Uri.parse(url),
    )
  ...
  Future<List<Songs>> get100TopSongs() async {
    ...
    final String url =
        "/v1/catalog/${AppleMusicToken.countryCode}/charts?types=songs&limit=100";
    return await appleMusicDio.dio
        .getUri(
      Uri.parse(url),
    )
  ...
  Future<List<Genre>> getGenres({int? limit}) async {
    ...
    final uri = _generateUrl(
        '/v1/catalog/${AppleMusicToken.countryCode}/genres', queryParameters);
  ...
  Future<Map<String, SearchParam>> searchTerm(String term) async {
    ...
    return await appleMusicDio.dio.getUri(Uri.parse(url)).then((value) {
      return (value.data['results'] as Map<String, dynamic>)
          .map((key, value){
